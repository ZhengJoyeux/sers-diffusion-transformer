...
OUTPUT_ROOT = Path(
    "outputs/experiments/"
    "d4_3_2_15_warmup_cosine_2h_formal/"
    "generated_support8_hybrid_guard_paired"
)
...
def main() -> None:
    ...
    for file_index, generated_path in enumerate(
        ...
        # ----------------------------------------------------
        # HYBRID lower-tail gate
        #
        # 普通异常仍要求 generated lower-rank gate。
        #
        # 但是，如果生成值已经低于：
        #
        #   同Raman位置training最低值
        #   -
        #   training-derived extrema margin
        #
        # 则说明已经突破真实training支持的逐点极值范围，
        # 此时允许绕过generated rank gate。
        # ----------------------------------------------------

        training_pointwise_minimum = np.min(
            training,
            axis=0,
        )

        extrema_floor = (
            training_pointwise_minimum
            - extrema_margin
        )
