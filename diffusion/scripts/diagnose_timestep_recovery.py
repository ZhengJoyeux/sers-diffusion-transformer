...
def main() -> None:
    ...
    # D4条件先验模型：
    # diagnostic_transform已经返回与当前真实光谱匹配的
    # deterministic prior。它既用于定义当前残差，
    # 也必须作为prior_conditioning输入U-Net。
    prior_conditioning_tensor = None

    if conditional_prior_bank is not None:
        if prior_normalized_2d is None:
            raise RuntimeError(
                "D4 timestep诊断缺少prior_normalized_2d。"
            )

        padded_prior_2d = length_adapter.adapt(
            np.asarray(
                prior_normalized_2d,
                dtype=np.float32,
            )
        ...
        if prior_conditioning_tensor.shape != x_start.shape:
            raise RuntimeError(
                "D4 timestep诊断prior_conditioning形状"
                "与x_start不一致："
                f"prior={tuple(prior_conditioning_tensor.shape)}, "
    ...
    with torch.inference_mode():
        for timestep in timesteps:
            ...
            if conditional_prior_bank is not None:
                if prior_conditioning_tensor is None:
                    raise RuntimeError(
                        "D4条件先验模型诊断缺少prior_conditioning。"
                    )
