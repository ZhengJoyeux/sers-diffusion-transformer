...
def main() -> None:
    ...
    # --------------------------------------------------------------
    # Generation-only sampling override
    #
    # The checkpoint remains the authoritative source for the trained
    # architecture and diffusion process.  The runtime YAML is allowed
    # to override ONLY sampling_timesteps so controlled sampler A/B
    # experiments can be performed without modifying the checkpoint.
    # --------------------------------------------------------------
    checkpoint_model_configuration = (
        checkpoint_configuration.get("model", {}) or {}
    )
    runtime_model_configuration = (
        configuration.get("model", {}) or {}
    )

    if not isinstance(checkpoint_model_configuration, dict):
        raise RuntimeError(
            "checkpoint中的model配置无效。"
        )
    if not isinstance(runtime_model_configuration, dict):
        raise RuntimeError(
            "runtime配置中的model配置无效。"
        )

    checkpoint_diffusion_timesteps = int(
        checkpoint_model_configuration.get(
            "diffusion_timesteps",
            checkpoint_model_configuration.get(
                "diffusion_steps",
                0,
    ...
    runtime_diffusion_timesteps = int(
        runtime_model_configuration.get(
            "diffusion_timesteps",
            runtime_model_configuration.get(
                "diffusion_steps",
                checkpoint_diffusion_timesteps,
            ),
        )
    )

    if runtime_diffusion_timesteps != checkpoint_diffusion_timesteps:
        raise RuntimeError(
            "生成阶段禁止修改diffusion_timesteps："
            f"checkpoint={checkpoint_diffusion_timesteps}，"
            f"runtime={runtime_diffusion_timesteps}。"
        )

    checkpoint_sampling_timesteps = int(
        checkpoint_model_configuration.get(
            "sampling_timesteps",
            checkpoint_model_configuration.get(
                "sampling_steps",
    ...
    runtime_sampling_timesteps = int(
        runtime_model_configuration.get(
            "sampling_timesteps",
            runtime_model_configuration.get(
                "sampling_steps",
                checkpoint_sampling_timesteps,
            ),
        )
    )
    ...
    if not (
        1
        <= runtime_sampling_timesteps
        <= checkpoint_diffusion_timesteps
    ):
        raise ValueError(
            "runtime sampling_timesteps必须位于"
            f"[1,{checkpoint_diffusion_timesteps}]，"
            f"实际={runtime_sampling_timesteps}。"
        )

    effective_model_configuration = dict(
        checkpoint_model_configuration
    )
    ...
    effective_checkpoint_configuration = dict(
        checkpoint_configuration
    )
    effective_checkpoint_configuration[
        "model"
    ] = effective_model_configuration
    ...
    #
    # model_builder实际使用diffusion_config，因此生成阶段runtime
    # override必须同步到两套兼容字段，否则会出现表面200、实际100。
    # ----------------------------------------------------------
    checkpoint_diffusion_configuration = dict(
        checkpoint_configuration.get("diffusion", {}) or {}
    )

    checkpoint_diffusion_steps = int(
        checkpoint_diffusion_configuration.get(
            "diffusion_steps",
            checkpoint_diffusion_configuration.get(
                "diffusion_timesteps",
                checkpoint_diffusion_timesteps,
    ...
    if checkpoint_diffusion_steps != checkpoint_diffusion_timesteps:
        raise RuntimeError(
            "checkpoint中model/diffusion总时间步不一致："
            f"model={checkpoint_diffusion_timesteps}；"
            f"diffusion={checkpoint_diffusion_steps}。"
        )

    effective_diffusion_configuration = dict(
        checkpoint_diffusion_configuration
    )

    # 两个名称都同步写入，避免不同版本builder读取不同别名。
    effective_diffusion_configuration[
        "sampling_steps"
    ] = runtime_sampling_timesteps
    ...
    print(
        "采样步数配置："
        f"checkpoint={checkpoint_sampling_timesteps}；"
        f"runtime={runtime_sampling_timesteps}；"
        f"effective={runtime_sampling_timesteps}"
    )

    _, diffusion = build_diffusion_model(
        model_configuration=effective_checkpoint_configuration,
        sequence_length=length_adapter.padded_length,
    )

    # ----------------------------------------------------------
    # Fail-fast：
    # 不再只相信配置文件，而直接检查最终构造出的diffusion对象。
    # ----------------------------------------------------------
    built_num_timesteps = int(
        getattr(diffusion, "num_timesteps", -1)
    )
    built_sampling_timesteps = int(
        getattr(diffusion, "sampling_timesteps", -1)
    )
    built_is_ddim = bool(
        getattr(diffusion, "is_ddim_sampling", False)
    )

    print(
        "实际扩散对象："
        f"num_timesteps={built_num_timesteps}；"
        f"sampling_timesteps={built_sampling_timesteps}；"
        f"is_ddim_sampling={built_is_ddim}"
    )

    if built_num_timesteps != checkpoint_diffusion_timesteps:
        raise RuntimeError(
            "最终扩散对象num_timesteps与checkpoint不一致："
            f"checkpoint={checkpoint_diffusion_timesteps}；"
            f"actual={built_num_timesteps}。"
        )

    if built_sampling_timesteps != runtime_sampling_timesteps:
        raise RuntimeError(
            "sampling_timesteps runtime override没有真正生效："
            f"requested={runtime_sampling_timesteps}；"
            f"actual={built_sampling_timesteps}。"
        )
    ...
    if built_is_ddim != expected_is_ddim:
        raise RuntimeError(
            "is_ddim_sampling状态异常："
            f"expected={expected_is_ddim}；"
            f"actual={built_is_ddim}。"
        )
