...
# D4.24-EXTENDED-DISTRIBUTION-METRICS-V2
#
# Whole-spectrum generation evaluation inspired by the evaluation
# structure used in DiffRaman.
#
# NOTE:
# - No fixed +/-50 cm-1 characteristic-peak window is used.
# - Existing peak morphology evaluation remains unchanged.
# - Distribution metrics use a train-only pointwise standardized space.
# - validation/test spectra never fit normalization parameters.


def _safe_profile_pearson(
    first: np.ndarray,
    second: np.ndarray,
) -> float:
    ...
    if (
        np.std(first_array, ddof=1) <= EPSILON
        or np.std(second_array, ddof=1) <= EPSILON
    ):
        return float("nan")
...
def _training_pointwise_standardization(
    training: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Fit condition-specific center/scale using training spectra only."""
...
def _apply_pointwise_standardization(
    spectra: np.ndarray,
    *,
    center: np.ndarray,
    scale: np.ndarray,
    ...
    if (
        values.ndim != 2
        or values.shape[1] != center.size
        or values.shape[1] != scale.size
        or not np.isfinite(values).all()
    ):
        raise ValueError(
            "光谱与training标准化状态不一致。"
    ...
    if not np.isfinite(
        ...
        raise RuntimeError(
            "training-standardized光谱出现NaN或无穷值。"
        )

    return standardized


def _all_pairs_cosine_mean(
    reference: np.ndarray,
    generated: np.ndarray,
) -> float:
    """Average cosine similarity over every generated-real pair."""
...
def _rbf_mmd(
    ...
    *,
    random_seed: int,
    maximum_generated_samples: int = 128,
) -> float:
    """RBF-kernel MMD with a deterministic median-bandwidth heuristic."""
    ...
    if (
        ...
        raise ValueError(
            "MMD要求reference/generated为同维二维数组。"
        )
    ...
    if (
        fake.shape[0]
        > maximum_generated_samples
    ):
        indices = rng.choice(
            fake.shape[0],
            size=maximum_generated_samples,
            replace=False,
        )
    ...
    pooled_distance = (
        _squared_distance_matrix(
            pooled,
            pooled,
        )
    )

    upper = pooled_distance[
        np.triu_indices(
            pooled.shape[0],
            k=1,
        )
    ...
    # Biased MMD^2:
    # stable and always non-negative up to floating-point error.
    mmd_squared = (
        float(
            np.mean(
                k_real_real
...
def _jensen_shannon_flattened(
    reference: np.ndarray,
    generated: np.ndarray,
    *,
    number_of_bins: int = 256,
) -> float:
    """JSD of the flattened standardized intensity distribution.

    Base-2 logarithm is used.
    """
...
def _sliced_wasserstein_distance(
    ...
    *,
    random_seed: int,
    number_of_projections: int = 64,
) -> float:
    """Whole-spectrum sliced Wasserstein distance."""
    ...
    if (
        ...
        raise ValueError(
            "SWD要求reference/generated为同维二维数组。"
        )
...
def _extended_distribution_metrics(
    *,
    training: np.ndarray,
    reference: np.ndarray,
    generated: np.ndarray,
    ...
    """Whole-spectrum COS/MMD/JSD/SWD metrics."""

    center, scale = (
        _training_pointwise_standardization(
            training
        )
    )

    reference_standardized = (
        _apply_pointwise_standardization(
            reference,
            center=center,
            scale=scale,
        )
    ...
    return {
        "difframan_all_pairs_cosine_raw": (
            _all_pairs_cosine_mean(
                reference,
                generated,
            )
        ),
        "mmd_rbf_training_standardized": (
            _rbf_mmd(
                reference_standardized,
                generated_standardized,
        ...
        "jsd_training_standardized_flattened": (
            _jensen_shannon_flattened(
                reference_standardized,
                generated_standardized,
            )
        ),
        "swd_training_standardized": (
            _sliced_wasserstein_distance(
                reference_standardized,
                generated_standardized,
                random_seed=random_seed,
...
def _std_profile_comparison(
    *,
    ...
    """Compare real and generated pointwise standard-deviation profiles."""
    ...
    return {
        f"{prefix}_std_profile_rmse_raw": float(
            np.sqrt(
                np.mean(
                    np.square(
                        difference
        ...
        f"{prefix}_std_profile_rmse_training_standardized": float(
            np.sqrt(
                np.mean(
                    np.square(
                        standardized_difference
        ...
        f"{prefix}_std_profile_ratio_median": float(
            np.median(
                ratio
            )
        ),
        f"{prefix}_std_profile_ratio_q10": float(
            np.quantile(
                ratio,
                0.10,
            )
        ),
        f"{prefix}_std_profile_ratio_q90": float(
            np.quantile(
                ratio,
                0.90,
            )
        ),
        f"{prefix}_std_profile_pearson": (
            _safe_profile_pearson(
                real_std,
                generated_std,
            )
        ),
...
def _mean_and_std_profile_metrics(
    *,
    training: np.ndarray,
    reference: np.ndarray,
    generated: np.ndarray,
) -> dict[str, float]:
    """Mean-spectrum fidelity and within-condition variation."""

    center, training_scale = (
        _training_pointwise_standardization(
            training
        )
    )
    ...
    result = {
        "mean_spectrum_rmse_raw_extended": float(
            np.sqrt(
                np.mean(
                    np.square(
                        mean_difference
        ...
        "mean_spectrum_rmse_training_standardized": float(
            np.sqrt(
                np.mean(
                    np.square(
                        mean_difference
        ...
        "mean_spectrum_cosine_extended": (
            _safe_profile_cosine(
                real_mean,
                generated_mean,
            )
        ),
        "mean_spectrum_pearson_extended": (
            _safe_profile_pearson(
                real_mean,
                generated_mean,
            )
...
def main() -> None:
    ...
    for condition_index, query in enumerate(conditions):
        ...
        distribution_metrics = (
            _extended_distribution_metrics(
                training=training,
                reference=reference,
                generated=generated,
                random_seed=2026 + condition_index,
        ...
        print(
            "  整体分布："
            f"all-pairs COS="
            f"{row['difframan_all_pairs_cosine_raw']:.6f}；"
            f"MMD="
            f"{row['mmd_rbf_training_standardized']:.6f}；"
            f"JSD="
            f"{row['jsd_training_standardized_flattened']:.6f}；"
            f"SWD="
            f"{row['swd_training_standardized']:.6f}"
        )
        print(
            "  类内std-profile："
            "training ratio/Pearson="
            f"{row['training_std_profile_ratio_median']:.4f}/"
            f"{row['training_std_profile_pearson']:.4f}；"
            "heldout ratio/Pearson="
            f"{row['heldout_std_profile_ratio_median']:.4f}/"
            f"{row['heldout_std_profile_pearson']:.4f}"
        )
        print(
            "  平均谱："
    ...
    # Ensure all newly added metrics are also present in
    # overall_metric_summary.csv even if _overall_summary()
    # uses an explicit legacy allow-list.
    extended_metric_names = (
        "difframan_all_pairs_cosine_raw",
        "mmd_rbf_training_standardized",
        "jsd_training_standardized_flattened",
        "swd_training_standardized",
        "mean_spectrum_rmse_raw_extended",
        "mean_spectrum_rmse_training_standardized",
        "mean_spectrum_cosine_extended",
        "mean_spectrum_pearson_extended",
        "heldout_std_profile_rmse_raw",
        "heldout_std_profile_rmse_training_standardized",
        "heldout_std_profile_ratio_median",
        "heldout_std_profile_ratio_q10",
        "heldout_std_profile_ratio_q90",
        "heldout_std_profile_pearson",
        "training_std_profile_rmse_raw",
        "training_std_profile_rmse_training_standardized",
        "training_std_profile_ratio_median",
        "training_std_profile_ratio_q10",
        "training_std_profile_ratio_q90",
        "training_std_profile_pearson",
    )

    existing_overall_metrics = set(
        overall["metric"].astype(str).tolist()
    )

    extended_overall_rows = []

    for metric_name in extended_metric_names:
        if (
            metric_name in existing_overall_metrics
            or metric_name not in condition_summary.columns
        ):
            continue
    ...
    report_lines = [
        ...
        "QQ: independently sorted quantiles; model-normalized QQ is retained as an auxiliary diagnostic",
        "Extended distribution metrics: all-pairs COS plus MMD/JSD/SWD; no fixed +/-50 cm-1 characteristic-peak window is used",
        "MMD/SWD: whole-spectrum comparison in train-only pointwise standardized space",
        "JSD: flattened intensity-distribution comparison in the same train-only standardized space",
        "Mean/std profile: mean-spectrum fidelity plus held-out/training pointwise variation profile agreement",
        "",
        ...
        "difframan_all_pairs_cosine_raw: "
        f"{overall_mean['difframan_all_pairs_cosine_raw']:.9g}",
        "mmd_rbf_training_standardized: "
        f"{overall_mean['mmd_rbf_training_standardized']:.9g}",
        "jsd_training_standardized_flattened: "
        f"{overall_mean['jsd_training_standardized_flattened']:.9g}",
        "swd_training_standardized: "
        f"{overall_mean['swd_training_standardized']:.9g}",
        "mean_spectrum_rmse_raw_extended: "
        f"{overall_mean['mean_spectrum_rmse_raw_extended']:.9g}",
        "mean_spectrum_rmse_training_standardized: "
        f"{overall_mean['mean_spectrum_rmse_training_standardized']:.9g}",
        "mean_spectrum_cosine_extended: "
        f"{overall_mean['mean_spectrum_cosine_extended']:.9g}",
        "mean_spectrum_pearson_extended: "
        f"{overall_mean['mean_spectrum_pearson_extended']:.9g}",
        "heldout_std_profile_rmse_training_standardized: "
        f"{overall_mean['heldout_std_profile_rmse_training_standardized']:.9g}",
        "heldout_std_profile_ratio_median: "
        f"{overall_mean['heldout_std_profile_ratio_median']:.9g}",
        "heldout_std_profile_pearson: "
        f"{overall_mean['heldout_std_profile_pearson']:.9g}",
        "training_std_profile_rmse_training_standardized: "
        f"{overall_mean['training_std_profile_rmse_training_standardized']:.9g}",
        "training_std_profile_ratio_median: "
        f"{overall_mean['training_std_profile_ratio_median']:.9g}",
        "training_std_profile_pearson: "
        f"{overall_mean['training_std_profile_pearson']:.9g}",
